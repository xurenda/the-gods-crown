...
export default class EventBus {
  events: Record<EventKey, Set<Function>>
   ...
   * @returns {this}
   */
  emit(eventKey: EventKey, ...args: any[]): this {
   ...
   * @returns {this}
   */
  on(eventKey: EventKey, cb: Function): this {
   ...
   * @returns {this}
   */
  once(eventKey: EventKey, cb: Function): this {
    // 包装方法：执行一次后取消订阅
    const func = (...args: any[]) => {
   ...
   * @returns {this}
   */
  off(eventKey: EventKey, cb?: Function): this {
    if (!cb) {
